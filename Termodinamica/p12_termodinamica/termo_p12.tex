...
Al comparar este resultado con el valor teórico que tiene el aluminio a temperatura ambiente ($K_{teo} \approx 237$~W$\cdot$m$^{-1}$K$^{-1}$)~\cite{Lunn2025}, se observa un error significativo, siendo la obtenida menor que la bibliográfica, para ser exactos unas 1.32 veces más pequeña aún habiendo usado la capacidad calorífica y densidad teórica.
...
Sustituyendo en la expresión general de propagación cuadrática, la \textbf{fórmula final de la incertidumbre} es:
\begin{equation}
    u_\lambda = \sqrt{ \left[ \frac{r(m^2 - h^2)}{2} u_K \right]^2 + \left[ \frac{K(m^2 - h^2)}{2} u_r \right]^2 + \left( K r h \cdot u_h \right)^2 + \left( -K r m \cdot u_m \right)^2 }
\end{equation}
