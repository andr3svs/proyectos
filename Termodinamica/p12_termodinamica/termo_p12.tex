...
\begin{itemize}
    ...
    \item $h = (8.6 \pm 1.3)\,\mathrm{m^{-1}}$
    \item $K_{exp} = (178.90 \pm 0.81)\,\mathrm{W\,m^{-1}\,K^{-1}}$
    \item $\lambda_{exp} = (64.3 \pm 5.8)\,\mathrm{W\,m^{-2}\,K^{-1}}$
\end{itemize}
...
Utilizando la distancia conocida entre los sensores ($d = 0.1000 \pm 0.0010$~m) y el radio ($r = 0.0200 \pm 0.0010$~m), la diferencia de fase extraída de los ajustes, se han calculado los parámetros cinemáticos de la onda térmica. 

Hemos obtenido el coeficiente $m = 10.45 \pm 0.60$~m$^{-1}$ y por otro lado, la otra constante obtenida es $h = 8.6 \pm 1.3$~m$^{-1}$. 

A partir de estos parámetros ondulatorios, se ha deducido el coeficiente de conductividad, obteniendo un valor de $\K_{exp} = 178.90 \pm 0.81$~W$\cdot$m$^{-1}$K$^{-1}$. 

Al comparar este resultado con el valor teórico que tiene el aluminio a temperatura ambiente ($\K_{teo} \approx 237$~W$\cdot$m$^{-1}$K$^{-1}$)~\cite{Lunn2025}, se observa un error significativo, siendo la obtenida menor que la bibliográfica, para ser exactos unas 1.32 veces más pequeña aún habiendo usando la capacidad calorífica y densidad teórica.
...
Con el resultado obtenido del coeficiente de conducción, hallaremos el coeficiente de pérdida lateral $\lambda$, que haciendo los cálculos correspondientes nos sale un valor de $\lambda_{exp} = (64.3 \pm 5.8)\,\mathrm{W\,m^{-2}\,K^{-1}}$. Este resultado no podemos compararlo con uno teórico para ver el error obtenido, pues este valor no es una propiedad del material, este es un valor que depende factores externos como puede ser la corriente de aire en el laboratorio. Podemos estimar que este valor también puede llegar a tener error. Nos da un valor un poco grande de pérdida lateral que se puede explicar el que sea un poco elevado debido al pequeño ventilador que tiene el montaje experimental creando esa convección con el aire, haciendo que la pérdida lateral aumente significsttiva respecto al aire en reposo.
...
A partir de la parte real de la ecuación diferencial del sistema, se obtiene la expresión para el coeficiente de pérdida de calor lateral que se define como:
\begin{equation}
    \lambda = \frac{K \cdot r}{2} (m^2 - h^2)
\end{equation}
...
\begin{align}
    \frac{\partial \lambda}{\partial K} &= \frac{r}{2} (m^2 - h^2) \\
    \frac{\partial \lambda}{\partial r} &= \frac{K}{2} (m^2 - h^2) \\
