...
\begin{itemize}
    \item $m = (10.45 \pm 0.60)\,\mathrm{m^{-1}}$
    \item $h = (22.9 \pm 1.3)\,\mathrm{m^{-1}}$
    \item $K_{exp} = (67.00 \pm 0.81)\,\mathrm{W\,m^{-1}\,K^{-1}}$
    \item $\lambda_{exp} = (2286.0 \pm 5.8)\,\mathrm{W\,m^{-2}\,K^{-1}}$
\end{itemize}
...
Utilizando la distancia conocida entre los sensores ($d = 0.1000 \pm 0.0025$~m) y el radio ($r = 0.0200 \pm 0.0025$~m), la diferencia de fase extraída de los ajustes, se han calculado los parámetros cinemáticos de la onda térmica. 

Hemos obtenido el coeficiente $m = 10.45 \pm 0.60$~m$^{-1}$ y por otro lado, la otra constante obtenida es $h = 22.9 \pm 1.3$~m$^{-1}$. 

A partir de estos parámetros ondulatorios, se ha deducido el coeficiente de conductividad, obteniendo un valor de $\K_{exp} = 67.00 \pm 0.81$~W$\cdot$m$^{-1}$K$^{-1}$. 

Al comparar este resultado con el valor teórico que tiene el aluminio a temperatura ambiente ($\K_{teo} \approx 237$~W$\cdot$m$^{-1}$K$^{-1}$)~\cite{Lunn2025}, se observa un error significativo, siendo la obtenida un muchísimo menos que la bibliográfica, para ser exactos unas 3.54 veces más pequeña aún habiendo usando la capacidad calorífica y densidad teórica.
...
\begin{itemize}
    ...
    \item La incertidumbre en la distancia se asume a partir del error de paralaje y la resolución de la regla graduada: $u_d = 0.0025$~m.
\end{itemize}
