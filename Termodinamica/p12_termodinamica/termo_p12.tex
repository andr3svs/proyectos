...
A partir de estos parámetros ondulatorios, se ha deducido la conductividad térmica experimental del material, obteniendo un valor de $\lambda_{exp} = 138.4 \pm 5.8$~W$\cdot$m$^{-1}$K$^{-1}$. 
...
No obstante, hemos decidido mencionar esto en próximo lugar ya que aunque es una explicación plausible, no tenemos forma de confirmarla y no tiene en cuenta los fallos del modelo,
...
\paragraph{3. Difusividad térmica ($K_{exp}$):}
A partir de $K_{exp} = \frac{\omega}{2 m h}$ (donde $\omega = 2\pi / \tau$), las derivadas parciales resultan:
\begin{align}
    \frac{\partial K_{exp}}{\partial \omega} &= \frac{1}{2 m h} \\
    \frac{\partial K_{exp}}{\partial m} &= -\frac{\omega}{2 m^2 h} \\
    \frac{\partial K_{exp}}{\partial h} &= -\frac{\omega}{2 m h^2}
\end{align}
\textit{Nota: La incertidumbre de la frecuencia angular se propaga como $u_{\omega} = \frac{2\pi}{\tau^2} u_{\tau}$.}

Sustituyendo, la \textbf{fórmula final de la incertidumbre} es:
\begin{equation}
    u_{K_{exp}} = \sqrt{ \left( \frac{u_{\omega}}{2 m h} \right)^2 + \left( -\frac{\omega}{2 m^2 h} u_m \right)^2 + \left( -\frac{\omega}{2 m h^2} u_h \right)^2 }
\end{equation}

\paragraph{4. Conductividad térmica ($\lambda_{exp}$):}
