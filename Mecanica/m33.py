...
# Analyze the voltage data to find local maxima and minima, ideally it would be alternating, but due to noise it may not be perfect.

for i in range(1, len(C_voltaje)-1):
    if C_voltaje[i]>C_voltaje[i-1] and C_voltaje[i]>C_voltaje[i+1]:
        print(f"Maximum voltage at index {i} with value {C_voltaje[i]} V")
        C_maximum_voltaje=np.append(C_maximum_voltaje, C_voltaje[i])
        C_number_maxima=np.append(C_number_maxima, i)
    elif C_voltaje[i]<C_voltaje[i-1] and C_voltaje[i]<C_voltaje[i+1]:
        print(f"Minimum voltage at index {i} with value {C_voltaje[i]} V")    
        C_minimum_voltaje=np.append(C_minimum_voltaje, C_voltaje[i])
        C_number_minima=np.append(C_number_minima, i)
...
C_distancias_cm = list(np.arange(0, 31.5, 0.5))
...
if len(C_number_maxima) >= 2:
    ...
    print(f"Fit MÁXIMOS: c = {c_fit_max:.2f} ± {sigma_c_max:.2f} m/s")
    ...
    plt.plot(n_line, s_max * n_line, '--', color='black', label=f'Máximos: c={c_fit_max:.1f} m/s')
    plt.xlabel('n (ordinal relativo)')
    plt.ylabel('Δd (m)')
    plt.title('Ajuste Δd = n*c/(4f) - MÁXIMOS')
    plt.legend()
    plt.show()
else:
    print('No hay suficientes máximos para ajustar.')
...
if len(C_number_minima) >= 2:
    ...
    print(f"Fit MÍNIMOS: c = {c_fit_min:.2f} ± {sigma_c_min:.2f} m/s")
    ...
    plt.plot(n_line, s_min * n_line, '--', color='black', label=f'Mínimos: c={c_fit_min:.1f} m/s')
    plt.xlabel('n (ordinal relativo)')
    plt.ylabel('Δd (m)')
    plt.title('Ajuste Δd = n*c/(4f) - MÍNIMOS')
    plt.legend()
    plt.show()
else:
    print('No hay suficientes mínimos para ajustar.')

# Comparar con teórica
theta = 25.0
c_theory = 331.3 * np.sqrt(1 + theta / 273.15)
print(f"\nVelocidad teórica a {theta} °C: c = {c_theory:.2f} m/s")
if len(C_number_maxima) >= 2:
    diff_max = (c_fit_max - c_theory) / c_theory * 100.0
    print(f"Diferencia MÁXIMOS: {diff_max:.2f} %")
if len(C_number_minima) >= 2:
    diff_min = (c_fit_min - c_theory) / c_theory * 100.0
    print(f"Diferencia MÍNIMOS: {diff_min:.2f} %")
